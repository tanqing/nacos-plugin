...
import com.alibaba.nacos.plugin.datasource.impl.postgresql.ConfigInfoTagMapperByPostgresql;
...
class ConfigInfoTagMapperByPostgreSqlTest {
    ...
    void testFindAllConfigInfoTagForDumpAllFetchRows() {
        ...
        assertEquals(mapperResult.getSql(), " SELECT t.id,data_id,group_id,tenant_id,tag_id,app_name,content,md5,gmt_modified  FROM (  "
                + "SELECT id FROM config_info_tag  ORDER BY id   OFFSET " + startRow + " LIMIT " + pageSize
